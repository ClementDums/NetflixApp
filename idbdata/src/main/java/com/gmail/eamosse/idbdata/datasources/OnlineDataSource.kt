...
internal class OnlineDataSource(private val service: MovieService) {
    ...
    suspend fun getDiscover(genre_id:Int, pageNumber:Int): Result<List<MovieResponse.MovieItem>> {
        return try {
    ...
    suspend fun getMovieVideos(movie_id: Int):Result<List<VideosMovieResponse.Result>>{
        return safeCall {
